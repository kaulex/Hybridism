...
class UneLigneStockItem extends React.Component{
    ...
    render (){
        ...
        const indice = this.props.unIndice

        for(let i = this.state.j; i < this.state.j+1; i)
        this.setState({j:this.state.j+1})

        console.log(unTabColor[indice[this.state.j]])
        ...
        return(    
         ...
         <View>
                <View style={styles.tabStockRef}>
                    ...
                    <Text style={{flex: 0.5, textAlign: 'right', padding: 10, fontSize: 12, color: ''}}>{ uneLigneStock.Tshirt_NbrStock } pièces</Text>

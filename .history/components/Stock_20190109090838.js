...
class Stock extends React.Component {
    ...
    constructor(props)
    {
        ...
        console.log(this.state.unStock.Tshirt_NbrStock)
        if(this.state.unStock.Tshirt_NbrStock < 5)
        {
            this.setState({couleur: '#f00'})
            console.log(this.state.couleur)
        }
        else
        {
            this.setState({couleur: '#000'})
            console.log(this.state.couleur)
        }
        });
        ...
        <View style={styles.container}>        
            ...
            <View style={styles.tableStock}>
                <FlatList
                    ...
                    renderItem={({item}) => <UneLigneStockItem uneLigneStock={item} couleurStock={this.state.couleur}/>}
                />
            </View>
        </View>
        </ImageBackground>
